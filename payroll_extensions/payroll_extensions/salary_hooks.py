...
def calculate_all_metrics(doc, method=None):
    ...
    frappe.db.set_value(doc.doctype, doc.name, "custom_late_entries", int(late_seconds))
    frappe.db.set_value(doc.doctype, doc.name, "custom_early_exits", int(early_seconds))
    frappe.db.set_value(doc.doctype, doc.name, "custom_extended_leaves", extended_leaves)
